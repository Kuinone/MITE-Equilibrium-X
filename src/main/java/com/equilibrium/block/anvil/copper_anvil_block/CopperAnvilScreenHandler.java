...
import static com.equilibrium.block.anvil.copper_anvil_block.CopperAnvilBlock.*;
import static com.equilibrium.util.AnvilPhase.getPhaseFromDurability;


public class CopperAnvilScreenHandler extends ItemCombinerMenu {
    ...
    public CopperAnvilScreenHandler(int syncId, Inventory inventory) {
        this(syncId, inventory, ContainerLevelAccess.NULL);
    }

    public CopperAnvilScreenHandler(int syncId, Inventory inventory, ContainerLevelAccess context) {
        super(ModBlockScreenTypesRegister.COPPER_ANVIL_SCREEN_TYPE,syncId,inventory,context);
    }
    ...
    protected boolean mayPickup(Player player, boolean present) {
        ...
        boolean shouldRejectForCopperAnvil = shouldRejectForCopperAnvil(this.inputSlots.getItem(0),this.inputSlots.getItem(1));

        return (preCondition && !additionalCondition && !shouldRejectForCopperAnvil);

    }

    private boolean shouldRejectForCopperAnvil(ItemStack input1, ItemStack input2) {
        boolean shouldReject1 = input1.is(ModItemTags.COPPER_ANVIL_REJECTION);
        boolean shouldReject2 = input2.is(ModItemTags.COPPER_ANVIL_REJECTION);
    ...
    protected void onTake(Player player, ItemStack stack) {
        ...
        this.access.execute((world, pos) -> {
                    ...
                    if (blockState.hasProperty(COPPER_ANVIL_DURABILITY_PROPERTY)) {
                        //铁砧目前的耐久
                        int i = blockState.getValue(COPPER_ANVIL_DURABILITY_PROPERTY);
                        //铁砧破坏进度
                        int phase = getPhaseFromDurability(COPPER_ANVIL_MAX_DURABILITY,blockState.getValue(COPPER_ANVIL_DURABILITY_PROPERTY));
                        //将耐久-1写入方块状态中,并更新外观状态
                        world.setBlockAndUpdate(pos,blockState
                                .setValue(com.equilibrium.block.anvil.copper_anvil_block.CopperAnvilBlock.FACING, blockState.getValue(com.equilibrium.block.anvil.copper_anvil_block.CopperAnvilBlock.FACING))
                                .setValue(COPPER_ANVIL_DURABILITY_PROPERTY,Math.clamp(i-1,0,COPPER_ANVIL_MAX_DURABILITY))
                                .setValue(COPPER_ANVIL_STAGE,Math.clamp(phase,0,2))
                        );
                    }
                    else
                        OnServerInitialize.LOGGER.error("No such Property called"+COPPER_ANVIL_DURABILITY_PROPERTY+ "or"+ COPPER_ANVIL_STAGE +"at the Anvil");
                }
        );
        ...
        this.access.execute((world, pos) -> {
            ...
            if (!player.hasInfiniteMaterials()) {
                //耐久为1时,直接损坏
                if(blockState.getValue(COPPER_ANVIL_DURABILITY_PROPERTY)==0) {
